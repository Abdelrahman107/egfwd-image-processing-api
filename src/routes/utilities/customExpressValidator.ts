...
const customQueryValidator: ValidationChain[] = [
  ...
  check('width').isLength({ min: 1 }).withMessage('width must be entered in query paramters').bail().isNumeric().withMessage('width must be a number').bail().isInt({ gt: 0 }).withMessage('width must be greater than 0'),
  check('height').isLength({ min: 1 }).withMessage('height must be entered in query paramters').bail().isNumeric().withMessage('height must be a number').bail().isInt({ gt: 0 }).withMessage('height must be greater than 0'),
];
