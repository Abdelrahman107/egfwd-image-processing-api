...
const customQueryValidator = [
    ...
    (0, express_validator_1.check)('width').isLength({ min: 1 }).withMessage('width must be entered in query paramters').bail().isNumeric().withMessage('width must be a number').bail().isInt({ gt: 0 }).withMessage('width must be intergar and greater than 0'),
    (0, express_validator_1.check)('height').isLength({ min: 1 }).withMessage('height must be entered in query paramters').bail().isNumeric().withMessage('height must be a number').bail().isInt({ gt: 0 }).withMessage('height must integer and greater than 0'),
];
